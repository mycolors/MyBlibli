...
public abstract class BaseTabFragment extends BaseFragment {
    ...
    private void initData() {
        ...
        for (int i = 0; i < fragments.length; i++) {
            fragments[i] = getChildFragmentManager().findFragmentByTag(getString(strings[i]));
        }
    }
    ...
    private void initView() {
        ...
        for (int i = 0; i < fragments.length; i++) {
            ...
            LinearLayout linear = (LinearLayout) getActivity().getLayoutInflater().inflate(R.layout.item_tab, linearLayout, false);
    ...
    private void onTabClick(int position) {
        ...
        selectTab(position);
        addOrShowFragment(getChildFragmentManager().beginTransaction(),
